...
# Page setup
st.set_page_config(
    page_title="Beach Profile Analysis Tool", 
    layout="wide"
)
...
if st.session_state.page == 'landing':
    st.title("Coastal Protection and Beach Nourishment Design Tool")
    st.image("images/bg.jpg", width="stretch")
    
    st.subheader("Engineering calculations, cross-section analysis, and cost estimation")
    ...
    with col1:
        ...
        st.markdown("""
        - **Region:** Ağlayankaya Beach, Şile
        - **Coordinates:** 41.175°N, 29.627°E
        - **Coastline Length:** ~480 meters
    ...
    with col2:
        ...
        - Bathymetry data extraction from NetCDF
        - Custom design profile input
        - Comparison graphs and analysis
        """)
    
    
    with st.form("start_form"):
        project_name = st.text_input("Project Name", value="Ağlayankaya Beach Nourishment")
        
        if st.form_submit_button("Start Analysis", type="primary", use_container_width=True):
...
# ===== PROJECT PAGE =====
    ...
    with col_back:
        if st.button("< Back to Home"):
            ...
            st.rerun()
    with col_title:
        st.title("Beach Profile Cross-Section Analysis")
        st.subheader(f"{st.session_state.get('project_name', 'Ağlayankaya Beach')} - Şile")
    ...
    if current == 'ALL':
        ...
        if not completed_sections:
            ...
            colors = {'A': '#2563EB', 'B': '#DC2626', 'C': '#FACC15'}  # Blue, Red, Yellow
        ...
        if len(section['points']) == 2:
            ...
            if section['bathy_dist']:
                ...
                if section['completed']:
                    ...
                    # Navigation buttons - centered and full width
